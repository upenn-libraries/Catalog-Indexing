...
describe ConfigItem do
  describe '.value_for' do
    ...
    it 'returns the value for the names config_item' do
      expect(described_class.value_for(config_item.name, detail_config: details)).to eq config_item.value
    end

    it 'raises and exception if a config_item is not found by that name' do
      expect {
        described_class.value_for(:nonexistent, detail_config: details)
